...
public partial class Academy_BusinessLicense : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["j"] != null && int.TryParse(TamperProofString.QueryStringDecode(Request.QueryString["j"]), out ajancyId)) // Edit mode
            {
                ...
                var ajancy = from j in db.Ajancies
                             ...
                             where j.AjancyID == ajancyId && ur.RoleID == (short)Public.Role.AcademyManager
                             select
                         new
                         {
                             j.AjancyID,
            ...
            if (Public.ActiveUserRole.RoleID == (short)Public.Role.Admin || Public.ActiveUserRole.RoleID == (short)Public.Role.AcademyProvince)
            {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (this.Page.IsValid)
        {
            ...
            if (this.ViewState["PersonID"] == null) // Add mode
            {
                ...
                usersInRole = new Ajancy.UsersInRole { RoleID = (short)Public.Role.AcademyManager, MembershipDate = DateTime.Now };
            ...
            else if (int.TryParse(this.ViewState["PersonID"].ToString(), out personId)) // Edit person mode
            {
                ...
                usersInRole = person.User.UsersInRoles.SingleOrDefault<Ajancy.UsersInRole>(ur => ur.RoleID == (short)Public.Role.AcademyManager);
                if (usersInRole == null)
                {
                    usersInRole = new Ajancy.UsersInRole { RoleID = (short)Public.Role.AcademyManager, MembershipDate = DateTime.Now };
            ...
            if (this.ViewState["AjancyID"] == null)
            {
                if (db.Ajancies.Any<Ajancy.Ajancy>(j => j.CityID == int.Parse(this.drpCity.SelectedValue) && j.AjancyType == (byte)Public.AjancyType.Academy && j.AjancyName.Equals(this.txtAjancyName.Text.Trim())))
                {
                    this.lblMessage.Text = string.Format("آموزشگاهی با نام <b>{0}</b> در شهرستان <b>{1}</b> موجود میباشد", ajancy.AjancyName, this.drpCity.SelectedItem.Text);
                    return;
                }
                ...
                {
                    this.lblMessage.Text = string.Format("آموزشگاهی با شماره پروانه کسب <b>{0}</b> در شهرستان <b>{1}</b> موجود میباشد", this.txtBusinessLicenseNo.Text, this.drpCity.SelectedItem.Text);
                    return;
                }
            ...
            else if (int.TryParse(this.ViewState["AjancyID"].ToString(), out ajancyId)) // Edit ajancy mode
            {
                ...
                {
                    this.lblMessage.Text = string.Format("آموزشگاهی با نام <b>{0}</b> در شهرستان <b>{1}</b> موجود میباشد", this.txtAjancyName.Text, this.drpCity.SelectedItem.Text);
                    return;
                }
            ...
            db.SubmitChanges();
            DisposeContext();
            Response.Redirect("~/Message.aspx?mode=18");
        }
    }
