...
public partial class Union_Driver : System.Web.UI.Page
{
    ...
    protected void txtNationalCode_TextChanged(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(this.txtNationalCode.Text))
        {
            ...
            if (person != null)
            {
                if (person.DriverCertifications.Any<Ajancy.DriverCertification>(dc => dc.CertificationType == (byte)Public.AjancyType.TaxiAjancy))
                {
                    // This person already has DriverCertification
                    Response.Redirect(string.Format("../Union/Driver.aspx?id={0}", TamperProofString.QueryStringEncode(person.PersonID.ToString())), true);
                }
            }
            SetPerson(person);
        }
